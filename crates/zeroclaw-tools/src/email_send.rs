...
zeroclaw_api::tool_attribution!(EmailSendTool, ToolKind::Plugin);
...
pub struct EmailSendTool {
    config: Arc<HashMap<String, EmailConfig>>,
}

impl EmailSendTool {
    pub fn new(config: Arc<HashMap<String, EmailConfig>>) -> Self {
        Self { config }
    }

    /// Helper to read the ZeroClaw configuration file from the default path
    fn get_smtp_config(&self) -> anyhow::Result<SmtpConfig> {
        let config = self
            .config
            .get("email_config")
            .ok_or_else(|| anyhow::Error::msg("Missing 'email_config' section in config"))?;
...
impl Tool for EmailSendTool {
    ...
    fn description(&self) -> &'static str {
        "Sends the email to specified recipients using the SMTP settings configured in config.toml."
    }
